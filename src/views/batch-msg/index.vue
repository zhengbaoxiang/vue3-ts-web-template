    ...
    <div class="batchContainer">
        <a-tabs :type="'line'" :size="'medium'" lazy-load v-model:active-key="curKey" default-active-key="draft"
            ...
            <a-tab-pane key="draft" title="草稿箱">
                <Draft ref="draftRef" :accountList="accountList" @toPreview="showPreview" />
            </a-tab-pane>
            <!-- 只有管理员和审核权限业务员 才能查看-->
            <a-tab-pane key="review" title="审核列表" v-if="role === 'Admin' || role === 'Auditor'">
                <Review ref="reviewRef" :accountList="accountList" @toPreview="showPreview" />
            </a-tab-pane>
            <a-tab-pane key="history" title="批量发送历史">
                <History ref="historyRef" :accountList="accountList" @toPreview="showPreview" />
            </a-tab-pane>
        </a-tabs>
        <FormDarwer ref="modalRef0" @refresh="refresh" @toPreview="showPreview"></FormDarwer>
...
const showPreview = (params) => {
    previewRef.value?.showAppPreview(params)
}
